        ...
        {pickups.data?.data.map((ticket) => (
          ...
          >
            ...
            <div className="mt-4 grid grid-cols-2 gap-3">
              <a
                className="rounded-xl border border-green-700 px-3 py-3 text-center text-sm font-bold text-green-800"
                href={mapsUrl(ticket)}
                rel="noreferrer"
                target="_blank"
              >
              ...
              {ticket.status === 'ASSIGNED' ? (
                <button
                  className="rounded-xl bg-green-700 px-3 py-3 text-sm font-bold text-white disabled:opacity-50"
                  disabled={start.isPending}
                  onClick={() => {
                    if (window.confirm('Mulai penjemputan ini?')) {
                ...
                <Link
                  className="rounded-xl bg-green-700 px-3 py-3 text-center text-sm font-bold text-white"
                  to={`/driver/pickups/${ticket.id}`}
                >
                  Lanjutkan
                </Link>
